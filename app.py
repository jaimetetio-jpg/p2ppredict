...
# Configuración de contraseña de administrador robusta
    ...
    try:
        if DATABASE_URL:
            ...
            c.execute(
                ...
                " username TEXT, tipo TEXT, monto DOUBLE PRECISION, txid TEXT, fecha TEXT )"
            )
            c.execute(
            ...
            c.execute(
                ...
                " AUTOINCREMENT, username TEXT, tipo TEXT, monto REAL, txid TEXT, fecha TEXT )"
            )
            c.execute(
            ...
            c.execute(
                ...
                " ip_address TEXT, user_agent TEXT, payload_snapshot TEXT, created_at TEXT )"
            )
            c.execute(
    ...
    try:
        ...
        if DATABASE_URL:
            c.execute(
                ...
                " ip_address, user_agent, payload_snapshot) VALUES (%s, %s, %s, %s, %s, %s)",
...
def obtener_saldo(username):
    ...
    try:
        ...
        if DATABASE_URL:
            ...
            c.execute(
                "SELECT * FROM historial_apuestas WHERE username = ? ORDER BY id DESC LIMIT ? OFFSET ?",
                (username, limite, offset),
            )
        ...
        if filtro_tipo:
            if DATABASE_URL:
                ...
                c.execute(
                    "SELECT * FROM transacciones WHERE username = ? AND tipo LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?",
                    (username, f"%{filtro_tipo}%", limite, offset),
                )
        else:
            if DATABASE_URL:
                ...
                c.execute(
                    "SELECT * FROM transacciones WHERE username = ? ORDER BY id DESC LIMIT ? OFFSET ?",
                    (username, limite, offset),
                )
...
def obtener_eventos():
    ...
    try:
        ...
        for ev in eventos_db:
            ...
            ev_dict["opciones"] = [dict(op) for op in c.fetchall()]
...
def pioneros_crear_prediccion():
    if not check_rate_limit(limit=10, window=60):
        return jsonify({
            "success": False,
            "error": "Demasiadas solicitudes. Por favor, espera un momento para volver a intentar.",
        }), 429
    ...
    if not username:
        return jsonify({"success": False, "error": "Se requiere autenticación"}), 400
    ...
    if not opcion_elegida:
        return jsonify({"success": False, "error": "Debe indicar la opción para la apuesta inicial."}), 400
    ...
    try:
        ...
        if not row or row.get("is_frozen"):
            conn.rollback()
            return jsonify({"success": False, "error": "Cuenta suspendida o no encontrada."}), 403

        saldo_actual = row["saldo_disponible"]
        if saldo_actual < liquidez_inicial:
            conn.rollback()
            return jsonify({"success": False, "error": "Saldo insuficiente para cubrir la apuesta inicial de 1 Pi"}), 400
        ...
        if DATABASE_URL:
            ...
            c.execute(
                "INSERT INTO eventos (titulo, categoria, estado, fecha_cierre) VALUES (?, ?, 'activo', ?)",
                (titulo, categoria, fecha_cierre),
            )
            ev_id = c.lastrowid
        ...
        for opt in opciones:
            pozo_inicial = liquidez_inicial if str(opt).strip() == str(opcion_elegida).strip() else 0.0
            if DATABASE_URL:
                c.execute(
                    "INSERT INTO opciones_evento (evento_id, nombre, pozo) VALUES (%s, %s, %s) RETURNING id",
        ...
        if DATABASE_URL:
            ...
            c.execute(
                "INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (%s, %s, %s, %s, %s)",
                (username, "Creación de Mercado (Apuesta Inicial de 1 Pi)", -liquidez_inicial, f"PIONEER_CREATE_{ev_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}", fecha_str),
            )
        else:
            c.execute(
        ...
        return jsonify({
            ...
            "mensaje": "¡Predicción creada con éxito con la apuesta inicial de 1 Pi!",
        })
    except Exception as e:
        conn.rollback()
        return jsonify({"success": False, "error": str(e)}), 500
...
def participar():
    ...
    try:
        ...
        if DATABASE_URL:
            ...
            c.execute("UPDATE opciones_evento SET pozo = pozo + %s WHERE id = %s", (monto_neto, opcion_id))
            c.execute(
            ...
            c.execute(
                "INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (%s, %s, %s, %s, %s)",
                (username, "Apuesta (con 2% comisión)", -monto_bruto, f"BET_{datetime.now().strftime('%Y%m%d%H%M%S')}", fecha_str),
            )
        else:
            c.execute("UPDATE usuarios SET saldo_disponible = ? WHERE username = ?", (nuevo_saldo, username))
            c.execute("UPDATE opciones_evento SET pozo = pozo + ? WHERE id = ?", (monto_neto, opcion_id))
            c.execute(
        ...
        return jsonify({
            ...
            "mensaje": "¡Apuesta registrada con éxito! (Comisión de plataforma del 2% aplicada)",
        })
    except Exception as e:
        conn.rollback()
        return jsonify({"success": False, "error": str(e)}), 500
...
def obtener_ordenes_clob():
    ...
    try:
        if evento_id:
            if DATABASE_URL:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = %s AND estado = 'activa' ORDER BY precio DESC",
                    (evento_id,),
                )
            else:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = ? AND estado = 'activa' ORDER BY precio DESC",
                    (evento_id,),
                )
        else:
...
def crear_orden_clob():
    ...
    try:
        ...
        if not row_user or row_user["saldo_disponible"] < costo_inicial:
            conn.rollback()
            return jsonify({"success": False, "error": "Saldo insuficiente para colocar la orden"})

        nuevo_saldo_creador = row_user["saldo_disponible"] - costo_inicial
        if DATABASE_URL:
            c.execute("UPDATE usuarios SET saldo_disponible = %s WHERE username = %s", (nuevo_saldo_creador, username))
        else:
        ...
        if accion == "comprar":
            if DATABASE_URL:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = %s AND opcion_id = %s AND accion = 'vender' AND estado = 'activa' AND precio <= %s ORDER BY precio ASC, id ASC FOR UPDATE",
                    (evento_id, opcion_id, precio),
                )
            else:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = ? AND opcion_id = ? AND accion = 'vender' AND estado = 'activa' AND precio <= ? ORDER BY precio ASC, id ASC",
                    (evento_id, opcion_id, precio),
                )
            ...
            for contra in contra_ordenes:
                ...
                if DATABASE_URL:
                    c.execute("SELECT saldo_disponible FROM usuarios WHERE username = %s FOR UPDATE", (contra["username"],))
                else:
                    c.execute("SELECT saldo_disponible FROM usuarios WHERE username = ?", (contra["username"],))
                v_row = c.fetchone()
                if v_row:
                    nuevo_saldo_vendedor = v_row["saldo_disponible"] + monto_vendedor
                    if DATABASE_URL:
                        c.execute("UPDATE usuarios SET saldo_disponible = %s WHERE username = %s", (nuevo_saldo_vendedor, contra["username"]))
                    else:
                ...
                if DATABASE_URL:
                    c.execute("UPDATE ordenes_clob SET cantidad = %s, estado = %s WHERE id = %s", (nueva_contra_cant, nuevo_estado_contra, contra["id"]))
                else:
                    c.execute("UPDATE ordenes_clob SET cantidad = ?, estado = ? WHERE id = ?", (nueva_contra_cant, nuevo_estado_contra, contra["id"]))
                cantidad_restante -= match_cant
        else:
            if DATABASE_URL:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = %s AND opcion_id = %s AND accion = 'comprar' AND estado = 'activa' AND precio >= %s ORDER BY precio DESC, id ASC FOR UPDATE",
                    (evento_id, opcion_id, precio),
                )
            else:
                c.execute(
                    "SELECT * FROM ordenes_clob WHERE evento_id = ? AND opcion_id = ? AND accion = 'comprar' AND estado = 'activa' AND precio >= ? ORDER BY precio DESC, id ASC",
                    (evento_id, opcion_id, precio),
                )
            contra_ordenes = c.fetchall()
            for contra in contra_ordenes:
        ...
        if cantidad_restante > 0:
            if DATABASE_URL:
                c.execute(
                    "INSERT INTO ordenes_clob (username, evento_id, opcion_id, tipo_orden, accion, precio, cantidad, estado, fecha) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (username, evento_id, opcion_id, tipo_orden, accion, precio, cantidad_restante, estado_final_orden, fecha_str),
                )
            else:
                c.execute(
                    "INSERT INTO ordenes_clob (username, evento_id, opcion_id, tipo_orden, accion, precio, cantidad, estado, fecha) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (username, evento_id, opcion_id, tipo_orden, accion, precio, cantidad_restante, estado_final_orden, fecha_str),
                )
        ...
        return jsonify({
            ...
            "mensaje": f"Orden procesada. Ejecutado: {cantidad - cantidad_restante} / Colocado en libro: {cantidad_restante}",
        })
    except Exception as e:
        conn.rollback()
        return jsonify({"success": False, "error": str(e)}), 500
...
def completar_pago():
    ...
    try:
        ...
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M")
        if DATABASE_URL:
            c.execute("INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (%s, %s, %s, %s, %s)", (username, "Recarga Pi Real", monto, txid or payment_id, fecha))
            ...
            c.execute("INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (?, ?, ?, ?, ?, ?)", (username, "Recarga Pi Real", monto, txid or payment_id, fecha))
            c.execute("SELECT SUM(saldo_disponible) as total FROM usuarios")
            res_tot = c.fetchone()
            balance_total_plataforma = res_tot["total"] if res_tot and res_tot["total"] else 0.0
            c.execute(
...
def solicitar_retiro():
    ...
    try:
        ...
        if DATABASE_URL:
            c.execute("INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (%s, %s, %s, %s, %s)", (username, "Retiro Pi Blockchain", -monto, txid, fecha))
            c.execute("SELECT SUM(saldo_disponible) as total FROM usuarios")
            res_tot = c.fetchone()
            balance_total_plataforma = res_tot["total"] if res_tot and res_tot["total"] else 0.0
            c.execute(
            ...
            c.execute("INSERT INTO transacciones (username, tipo, monto, txid, fecha) VALUES (?, ?, ?, ?, ?, ?)", (username, "Retiro Pi Blockchain", -monto, txid, fecha))
            c.execute("SELECT SUM(saldo_disponible) as total FROM usuarios")
            res_tot = c.fetchone()
            balance_total_plataforma = res_tot["total"] if res_tot and res_tot["total"] else 0.0
            c.execute(
...
def admin_login():
    if not check_rate_limit(limit=5, window=60):
        registrar_log_admin("LOGIN_FALLIDO_RATE_LIMIT", "Demasiados intentos de acceso bloqueados.")
...
def admin_cerrar_evento():
    ...
    try:
        ...
        if DATABASE_URL:
            ...
            c.execute(
                "SELECT * FROM historial_apuestas WHERE titulo_evento = ? AND opcion_elegida = ? AND estado = 'Activo'",
                (titulo_evento, nombre_ganador),
            )
        ...
        if DATABASE_URL:
            ...
            c.execute(
                "UPDATE historial_apuestas SET estado = 'Ganada' WHERE titulo_evento = ? AND opcion_elegida = ? AND estado = 'Activo'",
                (titulo_evento, nombre_ganador),
            )
            c.execute(
                "UPDATE historial_apuestas SET estado = 'Perdida' WHERE titulo_evento = ? AND opcion_elegida != ? AND estado = 'Activo'",
                (titulo_evento, nombre_ganador),
            )
        ...
        return jsonify({
            "success": True,
            "mensaje": f"Evento cerrado y premios acreditados automáticamente. Ganador: {nombre_ganador}",
        })
    except Exception as e:
        conn.rollback()
        return jsonify({"success": False, "error": str(e)}), 500
...
def admin_ajustar_balance():
    ...
    if not username:
        return jsonify({"success": False, "error": "Usuario no especificado"}), 400
    if not razon:
        return jsonify({"success": False, "error": "Es obligatorio dejar una nota o razón para el ajuste"}), 400
    ...
    try:
        ...
        if abs(monto_cambio) >= 100.0:
            ...
            if DATABASE_URL:
                c.execute(
                    "INSERT INTO admin_pending_actions (admin_creator, action_type, target_id, payload, status) VALUES (%s, %s, %s, %s, 'PENDING')",
                    ("Admin", "AJUSTE_BALANCE", username, payload_str),
                )
            else:
                fecha_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                c.execute(
                    "INSERT INTO admin_pending_actions (admin_creator, action_type, target_id, payload, status, created_at) VALUES (?, ?, ?, ?, 'PENDING', ?)",
            ...
            return jsonify({
                "success": True,
                "pending": True,
                "mensaje": "Ajuste crítico detectado. Solicitud retenida para aprobación dual.",
            })
...
def admin_anuncios():
    ...
    try:
        if request.method == "POST":
            ...
            if DATABASE_URL:
                c.execute(
                    "INSERT INTO anuncios_globales (titulo, contenido, tipo, activo, fecha) VALUES (%s, %s, %s, TRUE, %s)",
                    (titulo, contenido, tipo, fecha),
                )
            else:
                c.execute(
                    "INSERT INTO anuncios_globales (titulo, contenido, tipo, activo, fecha) VALUES (?, ?, ?, 1, ?)",
                    (titulo, contenido, tipo, fecha),
                )
            conn.commit()
